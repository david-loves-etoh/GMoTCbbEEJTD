...
const to_sequence = mountain=>mountain.map(column=>column[column.length-2].value)
...
const same_row = (entry1,entry2)=>!vertical_compare(entry1.y,entry2.y)

const vertical_increase = (y,d)=>{//go from y=[r0,r1,r2,...,r(d-1),rd #] to [0,0,0,...,0,rd+1 #]
      var c=y.slice()
...
const dimension_difference = (c1,c2)=>{
...
const create_entry = function(parent,entry){
      var newentry={
...
const find_higherequal = (column,y)=>{
      var i1=0,i2=column.length-1,i
      while(i1<i2){
         i=Math.ceil((i1+i2)/2)
...
const yslice = (column,lowequal,high)=>{
      var i1,i2,i
...
const collect_usual = (working_entry,collection=[])=>{
...
const collect1D = (working_entry,collection=[])=>{
...
const fill_magma_edge = (mountain,source_entry,leftleg_entry)=>{
      var newentry,d
...
const copy_single_edge = (mountain,source_entry,x_offset,BR_x,targety)=>{
...
const omega_Y_limit = (seq,FSterm)=>{
...
const displaywY = expr=>''+expr==='ω-Y(1,ω)'?'Limit':'ω-Y('+expr+')'