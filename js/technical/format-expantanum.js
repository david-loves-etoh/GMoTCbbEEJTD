// formatEN-expantanum.js by cloudytheconqueror
...
function regularFormatEN(num, precision) {
    ...
    let f = fmt.split(".")
    if (precision == 0) return commaFormatEN(num.floor ? num.floor() : Math.floor(num))
...
function polarizeEN(array, smallTop=false) {
    if (FORMAT_DEBUG_EN >= 1) console.log("Begin polarizeEN: "+JSON.stringify(array)+", smallTop "+smallTop)
    ...
    if (!Number.isFinite(bottom)) {}
        ...
        while (bottom >= 10 || elem < array.length || (smallTop && top >= 10)) {
            if (bottom >= 10) { // Bottom mode: the bottom number "climbs" to the top
                ...
                top += 1
                if (FORMAT_DEBUG_EN >= 1) console.log("Bottom mode: bottom "+bottom+", top "+top+", height "+height+", elem "+elem)
            }
                ...
                if (FORMAT_DEBUG_EN >= 1) console.log("Top mode: bottom "+bottom+", top "+top+", height "+height+", elem "+elem)
            }
        }
    }
    
    if (FORMAT_DEBUG_EN >= 1) console.log("Polarize result: bottom "+bottom+", top "+top+", height "+height)
...
function arraySearchEN(array, height) {
...
function setToZeroEN(array, height) {
    for (i=0;i<array.length;i++) {
...
function formatEN(num, precision=2, small=false) {
    ...
    if (num.sign < 0) return "-" + formatEN(num.neg(), precision)
    if (num.isInfinite()) return "Infinity"
    if (num.lt("0.0001")) { return formatEN(num.rec(), precision) + "⁻¹" }
    else if (num.lt(1)) return regularFormatEN(num, precision + (small ? 2 : 0))
    else if (num.lt(1000)) return regularFormatEN(num, precision)
    else if (num.lt(1e9)) return commaFormatEN(num)
    else if (num.lt("10^^5")) { // 1e9 ~ 1F5
        let bottom = arraySearchEN(array, 0)
        let rep = arraySearchEN(array, 1)-1
        if (bottom >= 1e9) {
            bottom = Math.log10(bottom)
            rep += 1
        }
        ...
        return "e".repeat(rep) + regularFormatEN(m, p) + "e" + commaFormatEN(e)
    }
    else if (num.lt("10^^1000000")) { // 1F5 ~ F1,000,000
        let pol = polarizeEN(array)
        return regularFormatEN(pol.bottom, precision3) + "F" + commaFormatEN(pol.top)
    }
    else if (num.lt("10^^^5")) { // F1,000,000 ~ 1G5
        let rep = arraySearchEN(array, 2)
        if (rep >= 1) {
            setToZeroEN(array, 2)
            return "F".repeat(rep) + formatEN(array, precision)
        }
        let n = arraySearchEN(array, 1) + 1
        if (num.gte("10^^" + (n + 1))) n += 1
        return "F" + formatEN(n, precision)
    }
        ...
        let rep = arraySearchEN(array, 3)
        if (rep >= 1) {
            setToZeroEN(array, 3)
            return "G".repeat(rep) + formatEN(array, precision)
        }
        let n = arraySearchEN(array, 2) + 1
        if (num.gte("10^^^" + (n + 1))) n += 1
        return "G" + formatEN(n, precision)
    }
        ...
        let rep = arraySearchEN(array, 4)
        if (rep >= 1) {
            setToZeroEN(array, 4)
            return "H".repeat(rep) + formatEN(array, precision)
        }
        let n = arraySearchEN(array, 3) + 1
        if (num.gte("10^^^^" + (n + 1))) n += 1
        return "H" + formatEN(n, precision)
    }
    else if (num.lt("J1000000")) { // 5J4 ~ J1,000,000
        let pol = polarizeEN(array, true)
        ...
        let rep = num.layer
        if (rep >= 1) return "J".repeat(rep) + formatEN(array, precision)
        let n = array[array.length-1][0]
        if (num.gte("J" + (n + 1))) n += 1
        return "J" + formatEN(n, precision)
    }
    ...
    return "K" + formatEN(n, precision)
}

function formatWholeEN(num) {
    return formatEN(num, 0)
}

function formatSmallEN(num, precision=2) { 
    return formatEN(num, precision, true)    
}